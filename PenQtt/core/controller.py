...
from core import network_scanner, sniffer, mqtt_enum, brute_force, fuzzer, qos_delay, dos_flodder, report
from core.global_state import sniffed_topics, brute_force_credentials
...
def run_cli_controller():
    ...
    if 1 <= choice <= len(iot_devices):
        ...
        brokers = sniffer.sniff_broker_from_iot(selected_ip, iface)
        ...
        if brokers:
            ...
            for broker_ip in brokers:
                ...
                if not sniffed_topics:
                    ...
                    if do_brute.lower() == 'y':
                        result = brute_force.mqtt_brute_force_combined(broker_ip)
                        if result:
                            mqtt_enum.mqtt_enum_broker(
                                broker_ip,
                                username=brute_force_credentials["username"],
                ...
                if do_qos.lower() == 'y':
                    qos_summary = qos_delay.mqtt_qos_delay_test(
                        broker_ip,
                        username=brute_force_credentials["username"],
                        password=brute_force_credentials["password"]
                    )
                ...
                if do_flood.lower() == 'y':
                    dos_flodder.mqtt_subscribe_publish(broker_ip)
