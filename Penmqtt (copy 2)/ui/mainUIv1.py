...
class PenMQTT(QMainWindow):
    ...
    def run_sequential_pentest(self, ip, device_name):
        try:
            ...
            self.start_automated_status_cycle()  # Mulai indikator siklus otomatis
            ...
            if not topics:
                ...
                if creds:
                    ...
                    self._update_attack_report("[!] Gagal brute force. Memeriksa input manual...\n")
                    # Ambil dari input user
                    username = self.id_input.text()
                    password = self.pass_input.text()
                    if username and password:
                        self.controller.credentials = (username, password)
                        self._update_attack_report(f"[✓] Menggunakan kredensial manual: {username}:{password}\n")
                        topics = enum.enum(broker_ip, username, password)
                    else:
                        self._update_attack_report("[!] Tidak ada kredensial manual dimasukkan. Pentest dibatalkan.\n")
            ...
            report = ReportGenerator(f"report_{broker_ip.replace('.', '_')}.pdf")
            report.generate(
                broker_ip=broker_ip,
